...
# App Configuration
# ==========================
st.set_page_config(page_title="Brain Tumor Detection", page_icon="🧠", layout="wide", initial_sidebar_state="expanded")
...
if uploaded_files:
    ...
    if len(uploaded_files) <= 5:
        ...
        for _idx, (col, file) in enumerate(zip(cols, uploaded_files)):
            with col:
                img = Image.open(file)
...
def draw_bboxes(image, detections, confidence_threshold=0.5):
    ...
    for det in detections:
        ...
        # Draw bounding box
        draw.rectangle([(x1, y1), (x2, y2)], fill=fill_color, outline=box_color, width=4)
        ...
        draw.rectangle([(x1, y1 - text_h - 8), (x1 + text_w + 12, y1)], fill=(0, 0, 0, 220))

        # Draw label text
        draw.text((x1 + 6, y1 - text_h - 6), label_text, fill=(255, 255, 255, 255))
...
if uploaded_files:
    ...
    with col2:
        run_detection = st.button("🔍 Run Detection Analysis", type="primary", use_container_width=True)
else:
    run_detection = False
            ...
            if response.status_code == 200:
                ...
                for idx, result in enumerate(results["results"]):
                    with st.container():
                        ...
                        # Load original image
                        original_image = Image.open(io.BytesIO(uploaded_files[idx].getvalue())).convert("RGB")

                        # Get detection summary
                        summary, status = get_detection_summary(result["detections"], confidence_threshold)

                        # Status badge
                        badge_class = "badge-positive" if status == "positive" else "badge-negative"
                        st.markdown(
                            f'<span class="detection-badge {badge_class}">{summary}</span>', unsafe_allow_html=True
                        )
                        ...
                        if result["detections"]:
                            ...
                            if filtered_detections:
                                ...
                                image_with_boxes = draw_bboxes(
                                    original_image.copy(), result["detections"], confidence_threshold
                                )
                                ...
                                if show_zoom:
                                    ...
                                    with col2:
                                        for det_idx, det in enumerate(filtered_detections[:3]):
                                            zoom_img = crop_zoom(original_image, det["bbox_xyxy"])
                                            st.image(
                                                zoom_img,
                            ...
                            st.image(original_image, caption="No Tumors Detected", use_column_width=True)
