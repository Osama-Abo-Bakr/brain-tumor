...
async def predict(files: list[UploadFile] = File(...)):
    ...
    for file in files:
        ...
        if not validate_image(file):
            ...
            failed_files.append({"filename": file.filename, "reason": f"Invalid file type: {file.content_type}"})
            continue
        ...
        try:
            ...
            if len(image_bytes) > 10 * 1024 * 1024:
                failed_files.append({"filename": file.filename, "reason": "File size exceeds 10MB"})
                continue
